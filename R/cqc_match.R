...
#'
#' cqc_update_match() matches the items from target groups to the reference by the named vector through 'map' argument.
#' cqc_remove_match() undo the matching that are produced either from cqc_update_match or cqc_match()
#' cqc_delete_unmatched() forces the unmatched items marked for deletion.
#'
...
#' @param map For cqc_update_match, it is a named vector that provides the paired strings(i.e. old value vs new value).
#'            For cqc_remove_match or cqc_delete_unmatched, it is a character vector that provides the items to be removed from matched pairs or mark
...
#' match_result <- cqc_update_match(match_result, map = c("PTPRC PE" = "CD45 PE"), group_id = 5)
#'
#' # You can delete any match added manually or automatically by cqc_match
#' match_result <- cqc_remove_match(match_result, "PTPRC PE", group_id = 5)
#'
...
cqc_update_match <- function(x, map, group_id = NULL){
  ...
  new_solution <- map_dfr(group_id, function(gid){
    ...
    if(any(idx))
    {
      ...
      stop("Found the existing match. Please call 'cqc_remove_match' to clear the existing match first")
    }
...
#' @export
#' @rdname cqc_update_match
cqc_delete_unmatched <- function(x, map, group_id = NULL){
  todel <- rep(NA, length(map))
  names(todel) <- map
  cqc_update_match(x, todel)

}
#' @export
#' @rdname cqc_update_match
cqc_remove_match <- function(x, map, group_id = NULL){
  if(!is(x, "cqc_match_result_and_solution"))
    stop("x must be the match result generated by cqc_match call!")
  if(length(names(map)) > 0)
