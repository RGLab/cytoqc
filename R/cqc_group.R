...
#' @examples
#' \dontrun{
#' groups <- cqc_prepare(cqc_cf_list, "channel")
#' }
#' @export
cqc_prepare <- function(x, ...) UseMethod("cqc_prepare")

#' @export
cqc_prepare.cqc_gs_list <- function(x, type = c("channel", "marker", "panel", "keyword", "gate"), delimiter = "|", ...) {
  ...
  res <- cqc_prepare(cflist, type, keys, delimiter, ...)
  # class(res) <- c("cqc_prepare_gs", class(res))
...
cqc_prepare.cqc_cf_list <- function(x, type = c("channel", "marker", "panel", "keyword"), keys = NULL, delimiter = "|", ...) {
  ...
  class(res) <- c("cqc_prepare", class(res))
  class(res) <- c(paste0("cqc_prepare_", type), class(res))
  attr(res, "data") <- x
  res
}

#' Provide the summary view of the cqc_prepare report.
#'
#' It summarise the sample-wise qc report into group overview report for reference selection and further QC action.
#'
#' @param object qc table returned by 'cqc_prepare'
...
#' }
#' @export
summary.cqc_prepare <- function(object, ...) {
  res <- object %>%
    select(-c(object)) %>%
    distinct()
  class(res) <- c("cqc_prepare_summary", class(res))
  res
}

#' @export
diff.cqc_prepare_gate <- function(x, ...) {
  diff.cqc_prepare(x, c("gate"))
}
#' @export
diff.cqc_prepare_keyword <- function(x, ...) {
  diff.cqc_prepare(x, c("keyword"))
}

#' @export
diff.cqc_prepare_channel <- function(x, ...) {
  diff.cqc_prepare(x, c("channel"))
}

#' @export
diff.cqc_prepare_marker <- function(x, ...) {
  diff.cqc_prepare(x, c("marker"))
}
#' @export
diff.cqc_prepare_panel <- function(x, ...) {
  diff.cqc_prepare(x, c("channel", "marker"))
}

#' Display the differences among QC groups
#'
#' @param x the grouped summary report generated by 'summary' call on the 'cqc_prepare' results
...
#' @export
diff.cqc_prepare <- function(x, vars, ...) {
  grps <- x %>%
...
#'
#' @param groups \code{cqc_prepare_gate} grouping resulte from \code{cqc_prepare}.
#' @export
...
#' Remove outlier groups from the result of 'cqc_prepare'.
#'
#' @param groups the object returned by 'cqc_prepares'
...
#' Extract the data from the result of a 'cqc_prepares' call.
#'
#' @param groups the object returned by 'cqc_prepares'
...
#'
#' @param x cqc report generated by 'cqc_prepare'
