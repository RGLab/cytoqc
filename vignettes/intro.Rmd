...
Firstly, determin the reference channels, which can be either manually supplied or automatically detected by helper function `cq_find_reference_params`
...
After reviewing the report, you can either manually fix these samples one by one
```{r}
cf <- cq_data[["s10a01.fcs"]]
colnames(cf)[1] <- "FSC-H"
```
Or use the helper tools we provided to correct them in batch. 

First, use the helper function to generate the fix solution 
```{r}
