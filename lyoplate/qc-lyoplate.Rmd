...
#group by gates
groups <- cqc_group(cqc_data, "gate")
su <- summary(groups)
knit_print(su)
knit_print(diff(su))
...
# match reference
match_result <- cqc_match_reference(groups, ref = 1, select = c(2))
match_result
...
summary(cqc_group(cqc_data, "gate"))
```
...
```{r}
res <- cqc_match_reference(groups, ref = 3, select = c(1,2,4,5,6)) 
res
...
```{r}
res <- cqc_match_reference(groups, ref = 2, select = c(1)) 
res

solution <- cqc_find_solution(res, max.distance = 0.8)
solution
cqc_fix(solution)
...
groups <- cqc_group(cqc_data, "panel")
knit_print(summary(groups))
```
...
## Remove `H/W` channels
```{r}
res <- cqc_match_reference(groups, ref = 4, select = c(1,2,3)) 
res
